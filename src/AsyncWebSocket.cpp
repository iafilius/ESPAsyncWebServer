...
size_t webSocketSendFrame(AsyncClient *client, bool final, uint8_t opcode,
                          bool mask, uint8_t *data, size_t len) {
  if (!client->canSend())
    return 0;
  size_t space = client->space();
  if (space < 2)
...
AsyncWebSocketMessageBuffer::AsyncWebSocketMessageBuffer(uint8_t *data,
                                                         size_t size)
...
AsyncWebSocketMessageBuffer::AsyncWebSocketMessageBuffer(
    const AsyncWebSocketMessageBuffer &copy)
    : _data(nullptr), _len(0), _lock(false), _count(0) {
  _len = copy._len;
  _lock = copy._lock;
  _count = 0;
...
AsyncWebSocketMessageBuffer::AsyncWebSocketMessageBuffer(
    AsyncWebSocketMessageBuffer &&copy)
    : _data(nullptr), _len(0), _lock(false), _count(0) {
  _len = copy._len;
  _lock = copy._lock;
  _count = 0;
...
public:
  AsyncWebSocketControl(uint8_t opcode, uint8_t *data = NULL, size_t len = 0,
                        bool mask = false)
...
AsyncWebSocketBasicMessage::AsyncWebSocketBasicMessage(const char *data,
                                                       size_t len,
                                                       uint8_t opcode,
                                                       bool mask)
...
AsyncWebSocketBasicMessage::AsyncWebSocketBasicMessage(uint8_t opcode,
                                                       bool mask)
...
size_t AsyncWebSocketBasicMessage::send(AsyncClient *client) {
  ...
  uint8_t opCode =
      (toSend && _sent == toSend) ? _opcode : (uint8_t)WS_CONTINUATION;
...
AsyncWebSocketMultiMessage::AsyncWebSocketMultiMessage(
    AsyncWebSocketMessageBuffer *buffer, uint8_t opcode, bool mask)
...
AsyncWebSocketClient::AsyncWebSocketClient(AsyncWebServerRequest *request,
                                           AsyncWebSocket *server)
    : _controlQueue(LinkedList<AsyncWebSocketControl *>(
          [](AsyncWebSocketControl *c) { delete c; })),
      _messageQueue(LinkedList<AsyncWebSocketMessage *>(
          [](AsyncWebSocketMessage *m) { delete m; })),
      _tempObject(NULL) {
...
void AsyncWebSocketClient::_onPoll() {
  if (_client->canSend() &&
      (!_controlQueue.isEmpty() || !_messageQueue.isEmpty())) {
    _runQueue();
  } else if (_keepAlivePeriod > 0 && _controlQueue.isEmpty() &&
...
void AsyncWebSocketClient::_runQueue() {
  ...
  if (!_controlQueue.isEmpty() &&
      (_messageQueue.isEmpty() || _messageQueue.front()->betweenFrames()) &&
      webSocketSendFrameWindow(_client) >
          (size_t)(_controlQueue.front()->len() - 1)) {
    _controlQueue.front()->send(_client);
  } else if (!_messageQueue.isEmpty() &&
             _messageQueue.front()->betweenFrames() &&
             webSocketSendFrameWindow(_client)) {
    _messageQueue.front()->send(_client);
  }
}

// should not be called if WS_MAX_QUEUED_MESSAGES set to < 0 (eg -1)
...
void AsyncWebSocketClient::_queueMessage(AsyncWebSocketMessage *dataMessage) {
  ...
  if ((WS_MAX_QUEUED_MESSAGES > 0) &&
      (_messageQueue.length() >= WS_MAX_QUEUED_MESSAGES)) {
...
void AsyncWebSocketClient::_queueControl(
...
void AsyncWebSocketClient::close(uint16_t code, const char *message) {
  ...
  if (code) {
    ...
    if (buf != NULL) {
      ...
      _queueControl(
          new AsyncWebSocketControl(WS_DISCONNECT, (uint8_t *)buf, packetLen));
      free(buf);
      return;
    }
  }
  ...
  while (plen > 0) {
    if (!_pstate) {
      ...
      if (_pinfo.len == 126) {
        ...
        _pinfo.len = fdata[9] | (uint16_t)(fdata[8]) << 8 |
                     (uint32_t)(fdata[7]) << 16 | (uint32_t)(fdata[6]) << 24 |
                     (uint64_t)(fdata[5]) << 32 | (uint64_t)(fdata[4]) << 40 |
    ...
    if ((datalen + _pinfo.index) < _pinfo.len) {
      ...
      _server->_handleEvent(this, WS_EVT_DATA, (void *)&_pinfo, (uint8_t *)data,
                            datalen);
      ...
      if (_pinfo.opcode == WS_DISCONNECT) {
        if (datalen) {
          ...
          if (reasonCode > 1001) {
            _server->_handleEvent(this, WS_EVT_ERROR, (void *)&reasonCode,
        ...
        if (_status == WS_DISCONNECTING) {
          ...
          _client->ackLater();
          _queueControl(
              new AsyncWebSocketControl(WS_DISCONNECT, data, datalen));
        }
      } else if (_pinfo.opcode == WS_PING) {
        _queueControl(new AsyncWebSocketControl(WS_PONG, data, datalen));
      } else if (_pinfo.opcode == WS_PONG) {
        if (datalen != AWSC_PING_PAYLOAD_LEN ||
            memcmp(AWSC_PING_PAYLOAD, data, AWSC_PING_PAYLOAD_LEN) != 0)
          _server->_handleEvent(this, WS_EVT_PONG, NULL, data, datalen);
      } else if (_pinfo.opcode < 8) { // continuation or text/binary frame
        _server->_handleEvent(this, WS_EVT_DATA, (void *)&_pinfo, data,
                              datalen);
      }
    } else {
      // os_printf("frame error: len: %u, index: %llu, total: %llu\n", datalen,
      // _pinfo.index, _pinfo.len); what should we do?
      break;
    }

    // restore byte as _handleEvent may have added a null terminator i.e.,
    // data[len] = 0;
    if (datalen > 0)
...
AsyncWebSocket::AsyncWebSocket(const String &url)
    : _url(url), _clients(LinkedList<AsyncWebSocketClient *>(
                     [](AsyncWebSocketClient *c) { delete c; })),
      _cNextId(1), _enabled(true),
      _buffers(LinkedList<AsyncWebSocketMessageBuffer *>(
          [](AsyncWebSocketMessageBuffer *b) { delete b; })) {
  _eventHandler = NULL;
}
...
void AsyncWebSocket::_handleDisconnect(AsyncWebSocketClient *client) {
  ...
  _clients.remove_first(
      [=](AsyncWebSocketClient *c) { return c->id() == client->id(); });
}
...
void AsyncWebSocket::binary(uint32_t id, const __FlashStringHelper *message,
                            size_t len) {
  AsyncWebSocketClient *c = client(id);
  if (c != NULL)
    c->binary(message, len);
...
bool AsyncWebSocket::canHandle(AsyncWebServerRequest *request) {
  ...
  if (request->method() != HTTP_GET || !request->url().equals(_url) ||
      !request->isExpectedRequestedConnType(RCT_WS))
    return false;
...
void AsyncWebSocket::handleRequest(AsyncWebServerRequest *request) {
  ...
  AsyncWebServerResponse *response =
      new AsyncWebSocketResponse(key->value(), this);
...
AsyncWebSocketMessageBuffer *AsyncWebSocket::makeBuffer(uint8_t *data,
                                                        size_t size) {
  AsyncWebSocketMessageBuffer *buffer =
      new AsyncWebSocketMessageBuffer(data, size);
...
AsyncWebSocket::AsyncWebSocketClientLinkedList
AsyncWebSocket::getClients() const {
  return _clients;
}

/*
 * Response to Web Socket request - sends the authorization and detaches the TCP
 * Client from the web server Authentication code from
 * https://github.com/Links2004/arduinoWebSockets/blob/master/src/WebSockets.cpp#L480
 */

AsyncWebSocketResponse::AsyncWebSocketResponse(const String &key,
                                               AsyncWebSocket *server) {
  _server = server;
  _code = 101;
...
size_t AsyncWebSocketResponse::_ack(AsyncWebServerRequest *request, size_t len,
                                    uint32_t time) {
  (void)time;
  if (len) {
